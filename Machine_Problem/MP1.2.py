"""
Enchanced Mini Expert System: University Logic Rules
...
def enrollment_rule(student_name):
    ...
    fees_paid = input("Has the student paid all fees? (T/F): ").strip().upper() == "T"
    enrolled = fees_paid   # Enrollment confirmed if fees are paid

    result = impl(fees_paid, enrolled)
    outcome = "Satisfied" if result else "Violated"

    print(f"P = {tf(fees_paid)} (Fees Paid), Q = {tf(enrolled)} (Enrollment Confirmed)")
    print("Result:", outcome)
...
def main():
    ...
    while True:
        ...
        print("5) Enrollment Clearance Checker")
        print("6) Exit")
        ...
        match choice:
            ...
            case "5": enrollment_rule(student_name)
            case "6":
